...
if (isset($_POST['submitted']) && $_POST['submitted'] == 1) {
	...
	if (!($_SESSION['Logged_In'] && ($_SESSION['IP'] == $_SERVER['REMOTE_ADDR']))) {
		...
		if ($Error == 0) {
			...
			if ($Project_ID > 0) {
				$query = $connection->prepare("INSERT INTO workunits (project_id, deadline, description, estimated_hours, status, type) VALUES (:project_id, :deadline, 'Projektleitung', 1, -1, 2)");
