...
if (!($_SESSION['Logged_In'] && ($_SESSION['IP'] == $_SERVER['REMOTE_ADDR']))) {
	...
	$query = $connection->prepare("SELECT project.creation, project.ID, project.description, project.deadline FROM project INNER JOIN worker_assignment on project.ID = worker_assignment.project_ID INNER JOIN workunits ON worker_assignment.workunit_ID = workunits.ID WHERE worker_assignment.user_ID = " . $_SESSION['user_id'] . " AND workunits.type != 3");
	$query->execute();
